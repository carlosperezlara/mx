...
TF1* GetFit(TString fout, TString fped, double xinit) {
  ifstream inped;
  inped.open( fped.Data() );
  double tmp;
  inped >> tmp;
  if(inped.good()) {
    cout << "PEDESTAL FILE FOUND: " << fped.Data() << endl;
    pedval = tmp;
  }
...
int display(int key=0) {
  ...
  */
  ...
  TF1 *fitH = GetFit( ou.Data(), ouP.Data(), xfit_min );
  ...
  hist->GetXaxis()->SetRangeUser(-5,125);
  ...
  text->DrawLatex(20, (0.83*(ymax)), Form("Entries  %.0f",entries) );
  //text->DrawLatex(30, (0.73*(ymax)), Form("State  %d",state) );
  text->DrawLatex(20, (0.53*(ymax)), Form("#lambda  %.1f #pm %.1f",lda,elda) );
  text->DrawLatex(20, (0.43*(ymax)), Form("#sigma  %.1f #pm %.1f",sg1,esg1) );
  text->SetTextColor(kRed-3);
  text->DrawLatex(20, (0.63*(ymax)), Form("#Chi^{2} / NDF  %.2f",ncs) );
  text->SetTextColor(kBlue-3);
  text->DrawLatex(55, (0.73*(ymax)), Form("#Alpha  %.1f M #pm %.0f",fitH->GetParameter(0)*1e-6,fitH->GetParError(0)) );
  text->SetTextColor(kCyan-3);
  text->DrawLatex(55, (0.63*(ymax)), Form("f_{1}  %.2f",fr1) );
  text->SetTextColor(kGreen-3);
  text->DrawLatex(55, (0.53*(ymax)), Form("f_{2}  %.2f #pm %.2f",fr2,efr2) );
  text->SetTextColor(kOrange-3);
  text->DrawLatex(55, (0.43*(ymax)), Form("f_{3}  %.2f #pm %.2f",fr3,efr3) );
  text->SetTextColor(kMagenta-3);
  text->DrawLatex(55, (0.33*(ymax)), Form("f_{4}  %.2f #pm %.2f",fr4,efr4) );
  text->SetTextColor(kGray);
  text->DrawLatex(55, (0.83*(ymax)), Form("b  %.2f",bsl) );
  text->SetTextColor(kBlack);
  text->SetTextSize(0.035);
  text->SetTextColor( kRed-3 );
  text->DrawLatex(20, (0.93*(ymax)), "e^{bx} + #Alpha ( f_{1} L_{1}(x) + f_{2} L_{2}(x) + f_{3} L_{3}(x) + f_{4} L_{4}(x))");
